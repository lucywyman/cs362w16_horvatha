...
int adventurerCard(struct gameState *state){
	...
	int y = state->discardCount[currentPlayer];//discard count
//printf("in adventurer \n");
	...
	//	printf("empty deck\n");
		...
		//printf("drawing card \n");
		drawCard(currentPlayer, state); //this call causes things to fail
		cardDrawn = state->hand[currentPlayer][state->handCount[currentPlayer]-1];//top card of hand is most recently drawn card.
		if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold){
	//	printf("Drew treasure \n");
		  drawntreasure++;
		  state->coins++; //Added to make adventurer easier to test
		}
		else{
	//	printf("didnt draw treasure \n");
		  temphand[z] = cardDrawn;
		  state->handCount[currentPlayer]--; //this should just remove the top card (the most recently drawn one).
		  z++;
		}
//printf("treasuer count %d coins %d\n", drawntreasure, state->coins);
	}
    while(z-1>=0){
...
//  int z = 0;// this is the counter for the temp hand
  ...
  switch( card ) 
    ...
    case mine:
		return mineCard(state, handPos, choice1, choice2);
		
    case remodel:
		return remodelCard(state, handPos, choice1, choice2);
